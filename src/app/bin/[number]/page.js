...
export async function generateMetadata({ params }) {
  ...
  if (data) {
    const title = `BIN ${bin} Details - ${data.brand || ""} ${data.type || ""} Card issued by ${data.bank || "Unknown Bank"} | BinX`;
    const description = `Verify BIN ${bin}. Country: ${data.country || "N/A"} ${data.flag || ""}, Network: ${data.brand || "N/A"}, Level: ${data.level || "N/A"}, Bank: ${data.bank || "N/A"}. Live lookup results from BinX.`;
    return {
      title,
      description,
      alternates: {
        canonical: `https://binx.vip/bin/${bin}`
      }
    };
  }

  return {
    title: `${bin} BIN Lookup - Verify Credit Card Info | BinX`,
    description: `Check details for BIN ${bin}. Find out the card brand, issuing bank, card level, and country of origin instantly with BinX.`,
    alternates: {
      canonical: `https://binx.vip/bin/${bin}`
    }
  };
...
export default async function SingleBinPage({ params }) {
  ...
  const getCardTheme = (brandName) => {
    ...
    if (brand.includes("visa")) {
      return {
        bg: "bg-gradient-to-br from-indigo-900 via-blue-900 to-indigo-950",
        overlay: "bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-400/20 via-transparent to-transparent",
        color: "text-blue-100",
        accent: "text-white",
        logo: "VISA"
      };
      ...
      return {
        bg: "bg-gradient-to-br from-red-950 via-rose-900 to-rose-950",
        overlay: "bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-orange-400/20 via-transparent to-transparent",
        color: "text-rose-100",
        accent: "text-white",
        logo: "Mastercard"
      };
      ...
      return {
        bg: "bg-gradient-to-br from-emerald-900 via-teal-900 to-emerald-950",
        overlay: "bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-teal-400/20 via-transparent to-transparent",
        color: "text-teal-100",
        accent: "text-white",
        logo: "AMEX"
      };
      ...
      return {
        ...
        overlay: "bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-fuchsia-400/20 via-transparent to-transparent",
      ...
      return {
        bg: "bg-gradient-to-br from-orange-900 via-amber-900 to-orange-950",
        overlay: "bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-amber-400/20 via-transparent to-transparent",
  ...
  return (
    <div className="w-full flex-grow flex flex-col animate-fade-up">
      ...
      <div className="mb-6">
        <a href="/" className="inline-flex items-center gap-2 text-xs font-bold text-gray-500 hover:text-gray-900 transition-colors">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="glass-panel p-6 sm:p-8 flex flex-col md:flex-row gap-8 items-center justify-between overflow-hidden relative group">
            ...
            <div className="flex-1 space-y-4 w-full">
              <span className="inline-flex items-center justify-center px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest bg-blue-100 text-blue-700 rounded">
                Live BIN Node
              </span>
              ...
              <p className="text-sm text-gray-500 leading-relaxed max-w-md">
                Verified index details of the card prefix range <span className="font-mono font-bold text-gray-900">{bin}</span>. This is {data.brand ? `a ${data.brand}` : 'an'} {data.type || "unknown type"} {data.level || "Classic"} card issued by <span className="font-bold text-gray-800">{data.bank || "Unknown Bank"}</span> in <span className="font-bold text-gray-800">{data.country || "Unknown Country"}</span>.
              </p>
            </div>
            ...
            <div className="relative w-full max-w-[340px] aspect-[1.58/1] shrink-0">
              ...
              <div className={`absolute inset-0 rounded-[18px] ${cardTheme.bg} p-6 flex flex-col justify-between border border-white/10 shadow-[0_15px_35px_-8px_rgba(0,0,0,0.4)] overflow-hidden group-hover:-translate-y-1 transition-transform duration-500`}>
                ...
                <div className="flex justify-between items-center z-10 my-1">
                  ...
                  <span className="text-sm font-black text-white/40 tracking-wider uppercase font-mono">
                    {cardTheme.logo}
                  </span>
                </div>
          ...
          <div className="glass-panel p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">BIN / IIN prefix</span>
                  <span className="font-mono font-bold text-gray-900 text-lg">{bin}</span>
                </div>
                <CopyButton text={bin} />
              </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Card Brand / Vendor</span>
                  <span className="font-bold text-gray-900 text-lg capitalize">{data.brand || "-"}</span>
                </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Funding Type</span>
                  <span className="font-bold text-gray-900 text-lg capitalize">{data.type || "-"}</span>
                </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Card Level / Category</span>
                  <span className="font-bold text-gray-900 text-lg capitalize">{data.level || "-"}</span>
                </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Luhn Checksum Algorithm</span>
                  <span className="font-bold text-green-600 flex items-center gap-1.5 text-lg">
                    <CheckCircle2 className="w-5 h-5 text-green-500 fill-green-50/50" /> Valid
                  </span>
                </div>
                <span className="text-[10px] bg-green-100 text-green-700 font-bold px-2 py-0.5 rounded uppercase">Compliant</span>
              </div>

              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between group/row">
                <div>
                  <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Country Origin</span>
                  <span className="font-bold text-gray-900 text-lg flex items-center gap-2">
                    {data.flag && <span className="text-xl">{data.flag}</span>}
                    {data.country || "-"} 
                    {data.countryCode && <span className="text-xs font-mono font-bold text-gray-400">({data.countryCode})</span>}
                  </span>
                </div>
            ...
            <div className="mt-6 p-5 bg-blue-50/30 rounded-2xl border border-blue-100/50 flex flex-col gap-6">
              <div>
                <span className="block text-[9px] font-bold text-blue-400 uppercase tracking-wider mb-1">Issuer Bank / Entity</span>
                <span className="font-bold text-blue-950 text-xl tracking-tight block">{data.bank || "Unknown Bank"}</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-blue-100/50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-blue-100/50 flex items-center justify-center text-blue-600 shrink-0">
                    <Globe className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <span className="block text-[8px] font-bold text-blue-400 uppercase tracking-wider">WEBSITE</span>
                    {data.url ? (
                      <a 
                        href={data.url.startsWith("http") ? data.url : `http://${data.url}`} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="font-bold text-blue-900 hover:text-blue-600 flex items-center gap-1 text-sm truncate"
                      >
                        {data.url}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <span className="block text-[8px] font-bold text-blue-400 uppercase tracking-wider">PHONE</span>
                    {data.phone ? (
                      <a 
                        href={`tel:${data.phone}`} 
                        className="font-bold text-blue-900 hover:text-blue-600 text-sm"
                      >
                        {data.phone}
                      </a>
                    ) : (
          ...
          <div className="glass-panel p-6 sm:p-8 space-y-4">
            <h3 className="text-base font-bold text-gray-900">About card prefix {bin}</h3>
            <div className="text-sm text-gray-600 space-y-3 leading-relaxed">
              ...
              <p>
                As a card range complying with ISO/IEC 7812 specifications, this prefix is checked against the Luhn validation formula and conforms to standardized routing paths. For transactions, the processor routes the authorization request to {data.bank || "the issuer bank"} via the {data.brand || "card vendor"} system networks.
              </p>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 sm:p-8">
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-2">
              ...
              Collections Including BIN {bin}
            </h2>
            <div className="space-y-3.5">
              <div className="p-4 border border-gray-200/60 rounded-xl hover:border-blue-200 cursor-pointer transition-colors flex items-center justify-between bg-white/50 hover:bg-white">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-green-500"></div>
                  <span className="font-bold text-gray-800 text-sm">NON-VBV BIN LIST</span>
                </div>
                <span className="text-xs text-gray-400 font-mono">2026-05-18</span>
              </div>
              <div className="p-4 border border-gray-200/60 rounded-xl hover:border-blue-200 cursor-pointer transition-colors flex items-center justify-between bg-white/50 hover:bg-white">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-purple-500"></div>
                  <span className="font-bold text-gray-800 text-sm">HIGH QUALITY SNIFFER</span>
                </div>
                <span className="text-xs text-gray-400 font-mono">2026-05-15</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-gray-900">@planetkyc</span>
                  <span className="text-[10px] text-gray-400 font-mono">2 days ago</span>
                </div>
                <p className="text-xs text-gray-600 leading-relaxed">
                  Working perfectly for Apple Pay loads. Need to bypass 3DS manually though.
                </p>
              </div>
              
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-gray-900">@searchmotor</span>
                  <span className="text-[10px] text-gray-400 font-mono">5 days ago</span>
                </div>
                <p className="text-xs text-gray-600 leading-relaxed">
                  Hit and miss on Stripe, highly recommended for Shopify gateways.
                </p>
              </div>
            </div>
          </div>
